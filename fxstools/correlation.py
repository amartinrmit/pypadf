...
`       number of rows in input diffraction array
    ...
    def calculate_correlation(self):
        ...
        for i in np.arange(self.npatterns // self.nthreads):
            ...
            print(f" correlated: {i*self.nthreads+1} / {self.npatterns+1}; time: {stop-t0:.2}s (last {self.nth} patterns:, {stop-start:.2}s); time remaining: {(stop-t0)*((self.npatterns/(i*self.nthreads+1))-1):.2}s", end='\x1b\r')
